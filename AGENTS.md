...
(`manifests/service.pp`). It can also be fully removed (package purged,
service stopped/disabled) by setting `haveged::package_ensure` to `absent`
(`manifests/init.pp`), and the whole module can be switched off globally
via the `simp_options::haveged` toggle (`manifests/init.pp`).
...
- **`haveged` (`manifests/init.pp`)** — Public entry class; consumers
  `include 'haveged'`. Key parameters (`init.pp`):
  ...
  - `simplib::assert_metadata($module_name)` first (`init.pp`).
  - The **entire body is gated** on
    `simplib::lookup('simp_options::haveged', { 'default_value' => true })`
    (`init.pp`) — when that global toggle is `false`, the module manages
    nothing.
  - **Package-ensure canonicalization** (`init.pp`): `true`→`'present'`,
    `false`/`'absent'`→`'purged'`, otherwise passthrough, into `$_package_ensure`.
  - **Service-ensure canonicalization** (`init.pp`): if
  ...
  - **Containment + ordering** (`init.pp`): always
...
- **`haveged::package` (`manifests/package.pp`)** — Manages the package.
  Params default back from `haveged::*` via `defined()`/`getvar()`
  (`package.pp`): `$package_name` (else `'haveged'`) and `$package_ensure`
  (from `haveged::_package_ensure`, else `'present'`). Works around
  puppetlabs PUP-1295 (`package.pp`): on `RedHat` family a requested
  `'purged'` is downgraded to `'absent'` (`package.pp`), then
  `package { $package_name: ensure => $_package_ensure }` (`package.pp`).

- **`haveged::service` (`manifests/service.pp`)** — Manages the service.
  Params default back from `haveged::*` (`service.pp`): `$service_name`,
  ...
  `false`). **rngd guard** (`service.pp`): if the `haveged__rngd_enabled`
  fact is true and `$force_if_rngd_running` is false, the service is set
  `ensure => 'stopped', enable => 'mask'`. Otherwise it honors
  `$service_ensure`/`$service_enable` (`service.pp`).

- **`haveged::config` (`manifests/config.pp`)** — Writes the systemd
  drop-in. Params default back from `haveged::*` (`config.pp`). Builds an
  options hash `{ '-b' => buffer, '-d' => data_cache, '-i' => instr_cache,
  '-w' => wakeup }` (`config.pp`), strips `undef` values
  (`delete_undef_values`, `config.pp`), joins into a flag string
  (`join_keys_to_values` + `join`, `config.pp`), and renders a heredoc
  systemd `[Service]` override that clears the stock `ExecStart` and sets
  `ExecStart=/usr/sbin/haveged --Foreground --verbose=1 ${_opts}`
  (`config.pp`). It is applied via
...
- **`absent` means purge.** `haveged::package_ensure` values `false` or
  `'absent'` canonicalize to `'purged'` (`init.pp`), which also forces the
  service stopped/disabled and skips `config` (`init.pp`). On RedHat-family
...
- **Sub-class parameter defaults are read from the parent** via
  ...
  (`package.pp`, `service.pp`, `config.pp`). The `service`
...
- **`config` uses stdlib 3.x-style function names** (`delete_undef_values`,
  `join_keys_to_values`, `join`, `config.pp`) rather than the namespaced
